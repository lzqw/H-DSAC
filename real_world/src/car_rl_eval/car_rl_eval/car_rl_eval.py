...
class CarRL(Node):
    ...
    def init_dpvp(self):
        ...
        self.load_apprfunc(self.config["apprfunc_path"])
    ...
    def sample(self):
        ...
        with torch.no_grad():
            ...
            action_out = action_distribution.mode()

            action = action.to('cpu').detach()[0].numpy()
            logp = logp.to('cpu').detach()[0].numpy()
            action_out=action_out.to('cpu').detach()[0].numpy()
        ...
        action_out_clip = action_out.clip(
            self.config["action_low_limit"], self.config["action_high_limit"]
        )

        print(self.turn)
        if self.turn==1 or self.turn==-1:
            if action_out_clip[0]>=0.20:
                print("changen")
                action_out_clip[0]=0.20
                self.high=True
                self.high_c=50
        else:
            if self.high==True and self.high_c!=0:
                print(self.high_c)
                if action_out_clip[0]>=0.20:
                    print("changen")
                    action_out_clip[0]=0.20
            ...
            if self.high_c==0:
                self.high=False
        self.send_action(action_out_clip)
        state_next=self.get_state()
        intervention=1-self.rcvMsgSurroundingInfo.car_run_mode
        print(self.rcvMsgSurroundingInfo.throttle_percentage)
        self.action_behavior=self.reverse_process_action(self.rcvMsgSurroundingInfo.throttle_percentage,
                                                    self.rcvMsgSurroundingInfo.braking_percentage,
                                                    self.rcvMsgSurroundingInfo.steerangle)
        print("action:",action_out_clip,self.action_behavior)
