...
from tensorboard_setup import tb_tags
from initialization import create_apprfunc
from common_utils import get_apprfunc_dict
from mlp import ActionValueDistri,StochaPolicy
...
class ApproxContainer(torch.nn.Module):
    ...
    def __init__(self, **kwargs):
        ...
        self.q1: nn.Module = ActionValueDistri(**q_args)
        self.q2: nn.Module = ActionValueDistri(**q_args)
        ...
        self.policy: nn.Module = StochaPolicy(**policy_args)
...
class DPVP:
    ...
    def __init__(self, **kwargs):
        ...
        self.tau = kwargs["tau"]
        self.target_entropy = -4*kwargs["action_dim"]
    ...
    def __compute_gradient(self, data: Dict, iteration: int):
        ...
        new_act = new_act.to(torch.float32)
        ...
        print("==============std================")
        print("DSAC2/policy_std-RL iter",policy_std)
        return tb_info
    ...
    def __compute_loss_q(self, data: Dict):
        ...
        action_novice, intervention, stop_td = (
            data["action_novice"],
            data["intervention"],
            data["stop_td"],
        )
        ...
        q1_loss_td = (torch.pow(self.networks.mean_std1_behavior, 2) + bias) * torch.mean(
            stop_td * (
                    ...
                    - ((torch.pow(q1_behavior.detach() - target_q1_behavior_bound, 2) - 5*q1_behavior_std_detach.pow(
        ...
        q2_loss_td = (torch.pow(self.networks.mean_std2_behavior, 2) + bias) * torch.mean(
            stop_td * (
                    ...
                    - ((torch.pow(q2_behavior.detach() - target_q2_behavior_bound, 2) - 5*q2_behavior_std_detach.pow(
        ...
        proxy1_loss_behavior = (torch.pow(self.networks.mean_std1_behavior, 2) + bias) * torch.mean(
            intervention * (
                    -(target_proxy1_behavior - q1_behavior).detach() / (
                        torch.pow(q1_behavior_std_detach, 2) + bias) * q1_behavior
                    - ((torch.pow(q1_behavior.detach() - target_proxy1_behavior_bound, 2) - 5*q1_behavior_std_detach.pow(
                2)) / (torch.pow(q1_behavior_std_detach, 3) + bias)
                       ) * q1_behavior_std
            ))
        ...
        proxy2_loss_behavior = (torch.pow(self.networks.mean_std2_behavior, 2) + bias) * torch.mean(
            intervention * (
                    -(target_proxy2_behavior - q2_behavior).detach() / (
                        torch.pow(q2_behavior_std_detach, 2) + bias) * q2_behavior
                    - ((torch.pow(q2_behavior.detach() - target_proxy2_behavior_bound, 2) - 5*q2_behavior_std_detach.pow(
                2)) / (torch.pow(q2_behavior_std_detach, 3) + bias)
                       ) * q2_behavior_std
            ))
    ...
    def __compute_target_q(self, done, q, q_std, q_next, q_next_sample, log_prob_a_next):
        target_q =  (1 - done) * self.gamma * (
                q_next - self.__get_alpha() * log_prob_a_next
        )
        target_q_sample = (1 - done) * self.gamma * (
    ...
    def __compute_loss_policy(self, data: Dict):
        ...
        loss_policy = (0.3*self.__get_alpha() * new_log_prob - torch.min(q1, q2)).mean()
